...
// 1. We must remove the us products from where we inserted them at the very end of the file.
// The usProducts string started with a comma, let's find it.
const searchStr = `  {
    id: "us-100-stack",
    slug: "us-100-stack",`;

const badIndex = fileContent.lastIndexOf(searchStr);
if (badIndex !== -1) {
    // We added `,` before it if there was no comma. Let's find the comma.
    const commaIndex = fileContent.lastIndexOf(',', badIndex);
    // Remove everything from the comma to the end except the final '];'
    fileContent = fileContent.slice(0, commaIndex) + '\n];\n';
}

// 2. Now let's inject it into PRODUCTS properly.
